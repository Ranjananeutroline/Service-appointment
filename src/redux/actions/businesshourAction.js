  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://www.localhost:8000/api/business/new",
        credentials
      );
  ...
  return async (dispatch)=>{
    try {
      ...
      const response = await axios.get(
        "http://www.localhost:8000/api/allHours",
...
export const deleteBusinesshoursByIDHere = (deleteId)=>{
  ...
  return async (dispatch) =>{
    try {
      const response = await axios.delete(`http://www.localhost:8000/api/business/${deleteId}`)
      console.log(response);
...
export const editBusinesshoursByIdHere = (data) => {
  return async (dispatch) => {
    try {    
       const response = await axios.patch(`http://www.localhost:8000/api/business/${data._id}`,data)
      console.log(response);
