  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://www.localhost:8000/api/announcement/offer/new",
        credentials
      );
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(
        "http://www.localhost:8000/api/announcement/allOffers"
      );
      console.log(response);
...
export const deleteOfferByIdHere = (id) =>{
  return async (dispatch) =>{
    try {
      console.log('yeha aayo ta????');
      const response = await axios.delete(`http://www.localhost:8000/api/announcement/offer/${id}`)
      console.log(response);
...
//       const response = await axios.delete(`http://www.localhost:8000/api/user/${deleteId}`);
//       console.log(response);
...
//       const response = await axios.patch(`http://www.localhost:8000/api/user/${appointmentId}`,{toNotify:true});
