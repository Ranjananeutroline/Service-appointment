  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://www.localhost:8000/api/service/new",
        credentials
      );
  ...
  return async (dispatch)=>{
    try {
      ...
      const response = await axios.get(
        "http://www.localhost:8000/api/service/allServices",
...
export const deleteServiceByIDHere = (deleteId)=>{
  ...
  return async (dispatch) =>{
    try {
      const response = await axios.delete(`http://www.localhost:8000/api/service/${deleteId}`)
      console.log(response);
...
export const editServiceByIdHere = (data) => {
  return async (dispatch) => {
    try {    
       const response = await axios.patch(`http://www.localhost:8000/api/service/${data._id}`,data)
      console.log(response);
