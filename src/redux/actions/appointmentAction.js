  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://www.localhost:8000/api/user/new",
        credentials
      );
  ...
  return async (dispatch)=>{
    try {
      ...
      const response = await axios.get(
        "http://www.localhost:8000/api/user/allAppointments",
...
export const confirmAppointmentRequestHere =(commitId)=>{
  ...
  return async (dispatch)=>{
    try {
      ...
      const response = await axios.patch(`http://www.localhost:8000/api/user/${commitId}`,{isRequestPending:false});
      console.log(response);
